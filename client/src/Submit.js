...
class FileFaxer extends React.Component {
  ...
  handleFileFax() {
    if (!this.props.selectedFile) {
      ...
      axios
        .post('/api/fax', formData, config)
        .then((response) => {
          console.log('Received successful fax response', response);

          this.setState({
            faxFileStatus:
              'Successfully faxed ' + response.data.FaxId + ' for $' + response.data.Price + '!',
          });
        })
        .catch((error) => {
          console.log(error);
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Button fullWidth variant="contained" color="primary" onClick={this.handleFileFax}>
          Fax me!
        </Button>
